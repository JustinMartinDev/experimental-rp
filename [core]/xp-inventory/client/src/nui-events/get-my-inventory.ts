import { triggerServerEvent, triggerClientEvent } from "@lib/citizenfx-utils/event/client";
...
const getMyInventory: NUICallbackFunction<unknown> = async (_data, cb) => {
  const { activeDbCharacterId } = await triggerClientEvent<GetActiveDbCharacterIdReturn>({
