...
const SelectCharacter = () => {
  ...
  const [activeCharacterId, setActiveCharacterId] = useState<undefined | number>();
  ...
  const handleSelectCharacter = async (id: string) => {
    ...
    try { 
      await fetchNui("set-active-character", "player-manager", { characterId: selectedCharacterId });
    } catch (err) {
      console.error(err);
    }
  ...
  const items = characters.map(({ firstname, lastname, id }) => ({
    title: id !== activeCharacterId ? `${firstname} ${lastname}` : `${firstname} ${lastname} (actuel)`,
    id: `${id}`,
  }));
